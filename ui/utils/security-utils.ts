...
const COMMON_WORDS = new Set([
  "configuration", "development", "production", "environment", "application",
  "authentication", "authorization", "credentials", "certificate", "parameter",
  "connection", "database", "password", "username", "localhost", "server",
  "client", "request", "response", "message", "error", "warning", "information",
  "success", "failure", "exception", "timeout", "interval", "duration"
])
...
function isLikelySensitive(value: string): boolean {
  return (
    value.length >= 12 &&
    calculateEntropy(value) > 3.5 &&
    !COMMON_WORDS.has(value.toLowerCase())
  )
}
