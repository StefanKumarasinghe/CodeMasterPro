...
export function CodeSast({ code, language, onClose }: CodeSastProps) {
  ...
  const runSastAnalysis = async () => {
    ...
    try {
      ...
      const { containedSensitiveInfo, detectedKeys } = processSensitiveCode(code)

      // Run SAST analysis
      const results = analyzeSast(code, language)

      // Add sensitive info detection to results if found
      ...
      <div className="bg-card border rounded-lg shadow-lg p-6 max-w-4xl w-full max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="gap-1" onClick={runSastAnalysis} disabled={isLoading}>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-8">
            ...
            <p className="text-sm text-muted-foreground">Running security analysis...</p>
          </div>
        ) : results ? (
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div
                className={cn(
                  ...
                  results.passed ? "bg-green-500/10 border-green-500/30" : "bg-red-500/10 border-red-500/30",
                )}
              >
              ...
              <div className="p-4 rounded-lg border">
                ...
                <div className="grid grid-cols-5 gap-2 text-center">
                  <div>
                    <div className="text-red-500 font-bold">{results.summary.blockers}</div>
                    <div className="text-xs text-muted-foreground">Blockers</div>
                  </div>
                  <div>
                    <div className="text-orange-500 font-bold">{results.summary.critical}</div>
                    <div className="text-xs text-muted-foreground">Critical</div>
                  </div>
                  <div>
                    <div className="text-yellow-500 font-bold">{results.summary.major}</div>
                    <div className="text-xs text-muted-foreground">Major</div>
                  </div>
                  <div>
                    <div className="text-blue-500 font-bold">{results.summary.minor}</div>
                    <div className="text-xs text-muted-foreground">Minor</div>
                  </div>
                  <div>
                    <div className="text-gray-500 font-bold">{results.summary.info}</div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
              <TabsList className="mb-4">
                <TabsTrigger value="issues">Issues ({results.summary.total})</TabsTrigger>
                ...
                <ScrollArea className="h-[400px]">
                  {results.issues.length > 0 ? (
                    <div className="space-y-2">
                      <div className="border rounded-md overflow-hidden">
                        {results.issues.map((issue: any, index: number) => (
                          <div
                            key={index}
                            className={cn("p-3 border-b last:border-b-0", getSeverityBgColor(issue.severity))}
                          >
                            <div className="flex items-start gap-2">
                              <div className="mt-0.5">
                                {issue.severity === "blocker" || issue.severity === "critical" ? (
                                  ...
                                  <Info className={cn("h-4 w-4", getSeverityColor(issue.severity))} />
                                )}
                              </div>
                    ...
                    <div className="bg-green-500/10 border border-green-200 rounded-md p-4 text-center">
                      ...
                      <p className="text-sm font-medium">No issues found in your code!</p>
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="border rounded-md p-4">
                        <h3 className="text-sm font-medium mb-3">Reliability ({results.metrics.reliability}%)</h3>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Reliability measures how well the code functions without bugs or unexpected behavior.
                        </p>
                      </div>

                      <div className="border rounded-md p-4">
                        <h3 className="text-sm font-medium mb-3">Security ({results.metrics.security}%)</h3>
                        <div className="h-2 bg-muted rounded-full mb-4">
                          <div
                            className={cn(
                              "h-full rounded-full",
                        ...
                        <p className="text-sm text-muted-foreground">
                          Security measures how well the code protects against vulnerabilities and attacks.
                        </p>
                      </div>
                      ...
                      <div className="border rounded-md p-4">
                        ...
                        <div className="h-2 bg-muted rounded-full mb-4">
                          <div
                            ...
                            style={{ width: `${results.metrics.maintainability}%` }}
                          />
                        </div>
                        <p className="text-sm text-muted-foreground">
                          Maintainability measures how easy it is to understand, modify, and extend the code.
                        </p>
                      </div>
                      ...
                      <div className="border rounded-md p-4">
                        ...
                        <div className="h-2 bg-muted rounded-full mb-4">
                          <div
                            ...
                            style={{ width: `${Math.min(results.metrics.complexity * 10, 100)}%` }}
                          />
                        </div>
                        <p className="text-sm text-muted-foreground">
                          Complexity measures how difficult the code is to understand and maintain.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="border rounded-md p-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">{results.issues.length} rules</span>
                        </div>
                        <div className="flex justify-between text-sm">
                      ...
                      <ul className="space-y-2 mt-2">
                        {results.issues.length > 0 ? (
                          <>
                            {results.summary.blockers > 0 && (
                              <li className="flex items-start gap-2">
                                ...
                                <div>
                                  <p className="text-sm font-medium">Fix blocker issues immediately</p>
                                  <p className="text-xs text-muted-foreground">
                                    These issues pose significant security or reliability risks.
                                  </p>
                                </div>
                              </li>
                            )}
                            ...
                            {results.metrics.maintainability < 70 && (
                              <li className="flex items-start gap-2">
                                ...
                                <div>
                                  <p className="text-sm font-medium">Improve code maintainability</p>
                                  <p className="text-xs text-muted-foreground">
                            ...
                            {results.metrics.complexity > 5 && (
                              <li className="flex items-start gap-2">
                                <Info className="h-4 w-4 text-blue-500 mt-0.5" />
                                <div>
                                  <p className="text-sm font-medium">Reduce code complexity</p>
                                  <p className="text-xs text-muted-foreground">
                          ...
                          <li className="flex items-start gap-2">
                            ...
                            <div>
                              <p className="text-sm font-medium">No immediate actions needed</p>
                              <p className="text-xs text-muted-foreground">
                                Your code looks good! Continue following best practices.
                              </p>
                            </div>
                          </li>
                        )}
